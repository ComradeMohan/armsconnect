...
export default function DemoPage() {
  ...
  // Demo Student Data
  const demoProfile = {
    name: "GOWRABATHUNI MOHIT TEJ",
    regno: "192314024L",
    dob: "15/03/2006",
    program: "COMPUTER SCIENCE AND ENGINEERING",
    email: "mohittejgowra@gmail.com",
    mobile: "7305119114",
    cgpa: 8.4,
    profilePictureUrl: "1e9d488f-c8d2-4b5f-8b9f-b5fbb56eb21a192314024L.JPG",
    notifications: [
      {
        by: "Academic Office",
        datetime: "05/07/2026",
        message: "Semester 6 Course registration closes today. Please complete your registration and verify all grade sheets immediately."
      },
      {
        by: "Principal",
        datetime: "30/06/2026",
        message: "Placement training classes scheduled for all 2023 - 2027 Batch students on 29.06.2026 & 30.06.2026. Attendance is mandatory for all students on those days."
      },
      {
        by: "Principal",
        datetime: "26/06/2026",
        message: "This is to inform all students that college will remain closed from 29 June 2026 to 05 July 2026. The classes will commence on Monday, 06 July 2026 for both Day Scholars and Hostelers. Note: Attendance on the first day is mandatory for all students."
      },
      {
        by: "Principal",
        datetime: "18/06/2026",
        message: "Supplementary Exam Schedule - HS2 - June / July 2026 (2022 & Passed Out Batches): https://docs.google.com/spreadsheets/d/1Op0-Fhj-1JbwQUQk4x0q5susDvvFPXwu6WDzGevKVNk/edit?usp=sharing"
      }
    ],
    attendance: [
      {
  ...
  const getGradeDistribution = () => {
    ...
    demoProfile.courses.forEach((c: any) => {
  ...
  const doughnutSegments = getDoughnutSegments(demoProfile.courses);
  ...
  const sortedCourses = [...(demoProfile?.courses || [])];
  if (sortConfig !== null) {
  ...
  return (
    <div>
      <div className="dashboard-wrapper">
        ...
        <div className="sidebar">
          <div className="glass-panel profile-card-panel" style={{ padding: 0, overflow: "hidden" }}>
            {demoProfile.profilePictureUrl && !imgError ? (
              <div
              ...
              >
                <img
                  src={`https://arms.sse.saveetha.com/Content/ProfilePicture/${demoProfile.profilePictureUrl}`}
              ...
              >
                <div className="avatar-large" style={{ transform: "none", marginBottom: "10px", width: "60px", height: "60px", fontSize: "24px" }}>
                  {demoProfile.name ? demoProfile.name[0].toUpperCase() : "S"}
                </div>
            ...
            <div style={{ padding: "20px 25px 25px 25px" }}>
              <div className="info-list" style={{ marginTop: "0" }}>
                ...
                <div className="info-item">
                  ...
                  <div className="info-content">
                    <label>Email</label>
                    <span style={{ fontSize: "11px" }}>{demoProfile.email}</span>
                  </div>
                </div>
                <div className="info-item">
        ...
        <div className="main-content">
          ...
          <div className="top-bar">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <button onClick={handleLogout} className="logout-btn" style={{
                ...
                <i className="fas fa-sign-out-alt"></i> Exit Demo
              </button>
            </div>
          </div>
          ...
          <div className="stats-grid">
            <button className="glass-panel stat-card" onClick={() => {
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                ...
                <h4 style={{ fontSize: "16px", fontWeight: 800, margin: 0, lineHeight: 1 }}>{demoProfile.courses.length}</h4>
              </div>
            </button>
            <button className="glass-panel stat-card" onClick={() => {
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                ...
                <h4 style={{ fontSize: "16px", fontWeight: 800, margin: 0, lineHeight: 1 }}>{demoProfile.attendance.length}</h4>
              </div>
            </button>
            <button className="glass-panel stat-card"
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                ...
                <h4 style={{ fontSize: "16px", fontWeight: 800, margin: 0, lineHeight: 1 }}>{demoProfile.notifications.length}</h4>
              </div>
            </button>
          </div>
          ...
          <div className="glass-panel notifications-panel">
            ...
            <div className="notification-list">
              {demoProfile.notifications.map((note: any, idx: number) => (
          ...
          {(() => {
            ...
            const recentNotifs = demoProfile.notifications.filter((n: any) => {
          ...
          <div className="glass-panel attendance-insights-panel" id="attendance-section">
            ...
            {demoProfile.attendance && demoProfile.attendance.length > 0 ? (
              <div className="table-container">
                <table>
                  ...
                  <tbody>
                    {demoProfile.attendance.map((subject: any) => {
          ...
          <div className="glass-panel completed-courses-panel" id="grade-section">
            ...
            {demoProfile.courses && demoProfile.courses.length > 0 ? (
              <div className="bunk-planner-grid">
                ...
                <div className="grade-chart-container" style={{ borderLeft: "1px solid var(--border-glass)", paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "15px", justifyContent: "flex-start" }}>
                  <div style={{
                    ...
                    <div className="legend-grid-wrapper" style={{
                      display: "grid",
                      gridTemplateColumns: demoProfile.courses.length > 5 ? "1fr" : "1fr 1fr",
                      gap: "10px 15px",
                      width: "100%",
                      maxWidth: demoProfile.courses.length > 5 ? "280px" : "100%",
                      margin: "0 auto"
                    }}>
                      {["S", "A", "B", "C", "D", "E"].map((grade) => {
                        const count = gradeCounts[grade] || 0;
                        const pct = demoProfile.courses.length > 0 ? Math.round((count / demoProfile.courses.length) * 100) : 0;
      ...
      {/* Notifications Modal — rendered via portal directly into document.body
           to escape any parent overflow:hidden / transform stacking context */}
        ...
        <div className="notif-modal-overlay" onClick={() => setShowNotificationsModal(false)}>
          <div className="notif-modal-content" onClick={(e) => e.stopPropagation()} style={{ animation: "slideUpModal 0.35s cubic-bezier(0.16,1,0.3,1)" }}>
            ...
            <div className="notif-modal-body" style={{ overflowY: "auto", maxHeight: "65vh", overscrollBehavior: "contain", paddingBottom: "20px" }}>
              {demoProfile.notifications && demoProfile.notifications.map((note: any, idx: number) => (
      ...
      {mounted && showProfileModal && createPortal(
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "16px 24px 20px" }}>
              {demoProfile.profilePictureUrl && !imgError ? (
                ...
                <div style={{ width: "64px", height: "64px", borderRadius: "50%", background: "var(--primary-gradient)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "24px", fontWeight: 800, color: "white", flexShrink: 0 }}>
                  {demoProfile.name?.[0]?.toUpperCase() ?? "S"}
                </div>
              )}
              <div>
                <h3 style={{ margin: 0, fontSize: "18px", fontWeight: 800, color: "white" }}>{demoProfile.name}</h3>
                <p style={{ margin: "4px 0 0", fontSize: "12px", color: "rgba(255,255,255,0.5)", fontWeight: 600 }}>{demoProfile.regno}</p>
              </div>
            </div>
            ...
            {/* Info rows */}
            {[
              { icon: "fa-graduation-cap", label: "Program",       value: demoProfile.program },
              { icon: "fa-calendar-alt",   label: "Date of Birth", value: demoProfile.dob },
              { icon: "fa-envelope",        label: "Email",         value: demoProfile.email },
              { icon: "fa-phone",           label: "Mobile",        value: demoProfile.mobile },
              { icon: "fa-chart-line",      label: "CGPA",          value: demoProfile.cgpa },
            ].map((row, i) => (
